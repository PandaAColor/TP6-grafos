...
from monticulo import HeapMin, HeapMax
...
class Graph(List):
    ...
    def __init__(self, is_directed=False):
        self.add_criterion('value', Graph._order_by_value)
    ...
    def deep_sweep(self, value) -> None: #desde el vértice que se de hace el barrido
    ...
    def kruskal_max(self, origin_vertex):
            def search_in_forest(forest, value):
                for index, tree in enumerate(forest):
                    if value in tree:
                        return index
    ...
    def shortest_path(self, origin, destination):
        path = self.dijkstra(origin)

        info = {}  # {vertex: (distance, previous)}
        while path.size() > 0:
            vertex, dist, prev = path.pop()
            info[vertex] = (dist, prev)

        if destination not in info or info[destination][0] == float("inf"):
            return None, None

        #Reconstruir el camino de atras hacia adelante
        camino = []
        actual = destination
        peso_total = info[destination][0]
    ...
    def print_kruskal(self, kruskal_result):
        peso_total = 0
        for edge in kruskal_result.split(';'):
            origin, destination, weight = edge.split('-')
            print(f'origin: {origin} destination: {destination}')
            peso_total += int(weight)
        return(f'peso total:{peso_total}')
